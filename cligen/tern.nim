...
type
  NodeOb*[T] {.acyclic.} = object
    ch*: char
    cnt*: int         ## Count of keys covered by this node for ``uniquePfxPat``
    when T isnot void:
      val*: T
...
proc print*[T](p: Node[T], depth=0) = #2,1,0 gives std emoji-orientation
  ...
  echo repeat("  ", depth),cast[int](p)," ch: '",p.ch,"' cnt: ",p.cnt
  print(p.kid[1], depth + 1)
  print(p.kid[0], depth + 1) ]#
proc leaves[T](r:Node[T], key=""): iterator(): tuple[k: string, n: Node[T]] =
  iterator(): tuple[k: string, n: Node[T]] =
    if r != nil:
      for e in toItr(leaves(r.kid[0], key)): yield e
      if r.ch == NUL: yield (k: key, n: r)
      else:
        for e in toItr(leaves(r.kid[1], key & r.ch)): yield e
      for e in toItr(leaves(r.kid[2], key)): yield e
...
proc incl*(t: var Tern[void], key: string) = discard rawInsert(t, key)
  ## Includes ``key`` in ``t``.
...
iterator pairs*[T](t: Tern[T]): tuple[key: string, val: T] =
  ...
  for k, x in toItr(t.root.leaves): yield (k, x.val)
...
iterator keysWithPrefix*[T](t: Tern[T], prefix: string, longest=false): string =
  ...
  for k, x in toItr(rawPfx(t, prefix, longest).leaves(prefix)): yield k

iterator valuesWithPrefix*[T](t: Tern[T], prefix: string, longest=false): T =
  ## Yields all values of ``t`` for keys starting with ``prefix``.
  for k, x in toItr(rawPfx(t, prefix, longest).leaves(prefix)): yield x.val
...
iterator pairsWithPrefix*[T](t: Tern[T], prefix: string,
  ...
  for k, x in toItr(rawPfx(t, prefix, longest).leaves(prefix)): yield (k, x.val)
...
proc uniquePfxPats*(t: Tern[void], x: openArray[string], sep="*"): seq[string] =
  ## Return shortest unique prefixes of ``x`` assuming no "" & unique ``x[i]``.
  result.setLen x.len; for i, s in x: result[i] = t.uniquePfxPat(s, sep)

proc uniquePfxPats*(x: openArray[string], sep="*"): seq[string] =
  ## Return shortest unique prefixes in ``x`` assuming no "" & unique ``x[i]``.
